...
{
    public class MyPointLatLngAlt
    {
        public double Lng;
        public double Lat;
        public double Alt;

        public MyPointLatLngAlt()
        {

        }

        public MyPointLatLngAlt(MyPointLatLngAlt point)
        {
            Lng = point.Lng;
            Lat = point.Lat;
            Alt = point.Alt;
        ...
        public MyPointLatLngAlt(double lat, double lng, double alt)
        {
            Lat = lat;
            Lng = lng;
            Alt = alt;
        ...
        public static readonly MyPointLatLngAlt Zero = new MyPointLatLngAlt();
    }
    ...
    public class ubx_m8p : ICorrections
    {
        ...
        public void SetupM8P(ICommsSerial port, bool m8p_130plus = false, bool movingbase = false, bool radioLink = false)
        {
            ...
            if (!radioLink)
            {
                ...
                var packetBaudRateConfig = generate(0x6, 0x00, new byte[] { 0x01, 0x00, 0x00, 0x00, 0xD0, 0x08, 0x00, 0x00, 0x00, 0xC2,
                0x01, 0x00, 0x23, 0x00, 0x23, 0x00, 0x00, 0x00, 0x00, 0x00 });
                port.Write(packetBaudRateConfig, 0, packetBaudRateConfig.Length);
                System.Threading.Thread.Sleep(300);

                // port config - usb
                packetBaudRateConfig = generate(0x6, 0x00, new byte[] { 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                ...
                // port config - 115200 - uart1
                packetBaudRateConfig = generate(0x6, 0x00, new byte[] { 0x01, 0x00, 0x00, 0x00, 0xD0, 0x08, 0x00, 0x00, 0x00, 0xC2,
                0x01, 0x00, 0x23, 0x00, 0x23, 0x00, 0x00, 0x00, 0x00, 0x00 });
                port.Write(packetBaudRateConfig, 0, packetBaudRateConfig.Length);
                System.Threading.Thread.Sleep(300);
        ...
        public void SetupBasePos(ICommsSerial port, MyPointLatLngAlt basepos, int surveyindur = 0, double surveyinacc = 0, bool disable = false, bool movingbase = false)
        {
            if (movingbase)
                disable = true;
